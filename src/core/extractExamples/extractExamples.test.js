...
describe('extractExamples', () => {
    it('extracts correct single example from sense markup', () => {
        expect(extractExamples(senseWithSingleExampleMarkup)).toEqual([
            'The set (x, y) has two members.'
        ])
    })
    it('extracts correct examples from sense markup', () => {
        expect(extractExamples(senseMarkup)).toEqual([
    ...
    it('extracts correct examples from example group markup', () => {
        expect(extractExamples(exampleGroupMarkup)).toEqual([
    ...
    it('removes newline and double spaces', () => {
        expect(extractExamples(senseWithExamplesWithDoubleSpaces)[4]).toEqual(

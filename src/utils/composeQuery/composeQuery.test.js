...
const beforeWord =
    'https://query.yahooapis.com/v1/public/yql?q=SELECT%20*%20FROM%20htmlstring%20WHERE%20url%3D%22http%3A%2F%2Fwww.ldoceonline.com%2Fdictionary%2F'
...
describe('composeQuery', () => {
    it('composes correct query one simple word', () => {
        expect(composeQuery('set')).toEqual(addFrame('set'))
    })

    it('composes correct query for word with space', () => {
        expect(composeQuery('fire alarm')).toEqual(addFrame('fire-alarm'))
    })
    ...
    it('composes correct query for word with exclamation mark', () => {
        expect(composeQuery('that does it!')).toEqual(addFrame('that-does-it'))
    })
